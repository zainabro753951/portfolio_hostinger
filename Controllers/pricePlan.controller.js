...
export const AddPlanValidation = [
  ...
  body("featurePoints")
    .isArray({ min: 1 })
    .withMessage("At least one feature point is required!"),
...
export const addPricePlan = async (req, res) => {
  ...
  try {
    if (typeof featurePoints === "string")
      featurePoints = safeParse(featurePoints);

    // ✅ If it's UPDATE mode
    if (isUpdate && planId) {
      // check if plan exists
      const [existing] = await pool.query("SELECT * FROM plans WHERE id = ?", [
        planId,
      ]);
      if (!existing.length) {
      ...
      const [duplicate] = await pool.query(
        "SELECT 1 FROM plans WHERE planName = ? AND id != ?",
        [planName, planId],
      );
    ...
    const [exists] = await pool.query(
      "SELECT 1 FROM plans WHERE planName = ?",
      [planName],
    );
    if (exists.length > 0) {
      return res.status(400).json({
...
export const getPricePlan = async (req, res) => {
  try {
    ...
    // Mapping over plans to parse featurePoints
    const formattedPlans = plans.map((item) => ({
      ...item,
      featurePoints: JSON.parse(item.featurePoints),
    }));
